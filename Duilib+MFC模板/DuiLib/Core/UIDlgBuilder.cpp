...
CDialogBuilder::CDialogBuilder(int nLangIndex_) : m_pCallback(NULL), m_pstrtype(NULL),m_nLangIndex(nLangIndex_)
{
}
...
CControlUI* CDialogBuilder::_Parse(CMarkupNode* pRoot, CControlUI* pParent, CPaintManagerUI* pManager)
{
    ...
    for( CMarkupNode node = pRoot->GetChild() ; node.IsValid(); node = node.GetSibling() ) {
        ...
        if( node.HasAttributes() ) {
            ...
            int nAttributes = node.GetAttributeCount();
            for( int i = 0; i < nAttributes; i++ ) {
				LPCTSTR _pszName = node.GetAttributeName(i);
				LPCTSTR _pszValue = node.GetAttributeValue(i);
				int _nLangIndex = -1;
				// 检查一下是否是tooltip%d属性，如果是的话，做一下拦截，将属性修改为tooltip，并读取后面的字符串值，设置其为tooltip的值 孙彦春 2017-1-18
				if (1 == _stscanf_s(_pszName, _T("tooltip%d"), &_nLangIndex) && _nLangIndex == m_nLangIndex)
					pControl->SetAttribute(_T("tooltip"), _pszValue);
				// 检查一下是否是text%d属性，如果是的话，做一下拦截，将属性修改为text，并读取后面的字符串值，设置其为text的值 孙彦春 2017-1-18
				else if(1 == _stscanf_s(_pszName, _T("text%d"), &_nLangIndex) && _nLangIndex == m_nLangIndex)
					pControl->SetAttribute(_T("text"), _pszValue);
				// 检查一下是否是font%d属性，如果是的话，做一下拦截，将属性修改为font，并读取后面的字符串值，设置其为font的值 孙彦春 2017-1-18
				else if(1 == _stscanf_s(_pszName, _T("font%d"), &_nLangIndex) && _nLangIndex == m_nLangIndex)
					pControl->SetAttribute(_T("font"), _pszValue);
				else
					pControl->SetAttribute(_pszName, _pszValue);
            }
        }
        if( pManager ) {
