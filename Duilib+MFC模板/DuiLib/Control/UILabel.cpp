...
namespace DuiLib
{
	...
	void CLabelUI::DoEvent(TEventUI& event)
	{
		...
		// ´Ëelse ifÓï¾ä¿½±´×ÔCButtonUIµÄDoEvent()º¯Êý ËïÑå´º 2016-10-7 14:51:24
	...
	// CLabelUI::Activate()µÄ´úÂë¿½±´×ÔCButtonUIµÄActivate()º¯Êý ËïÑå´º 2016-10-7 14:50:26
	bool CLabelUI::Activate()
	{
	...
	void CLabelUI::SetFloatNum(double nNum_, int nPresion_/* = 3*/)
	{
		CDuiString _str;
		_str.Format(_T("%.*lf"), nPresion_, nNum_);
		SetText(_str);
	}
