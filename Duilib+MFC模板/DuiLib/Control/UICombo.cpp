...
CListLabelElementUI* CComboUI::GetCurSelElement() const
{
	CControlUI* _pControlUI = GetItemAt(m_iCurSel);
	return dynamic_cast<CListLabelElementUI*>(_pControlUI);
}
...
bool CComboUI::SelectItem(int iIndex, bool bTakeFocus)
{
	...
	// 孙彦春修改，不可见就不能更改选项，我觉得这个不合理，不可见的原因有可能只是被其他页面遮挡 孙彦春 2016-5-25 15:49:22
...
// 孙彦春补充 孙彦春 2016-6-19 17:11:11
bool CComboUI::SelectItem(LPCTSTR pszText_, bool bTakeFocus/* = false*/)
{
	const int c_nListSize = m_items.GetSize();
	for (int _i = 0; _i != c_nListSize; ++_i)
	{
		CControlUI* pControl = static_cast<CControlUI*>(m_items[_i]);
		if( !pControl ) return false;
		CDuiString _strText = pControl->GetText();
		if (_strText.Compare(pszText_) == 0)
		{
			return SelectItem(_i, bTakeFocus);
		}
	}
	return SelectItem(-1, bTakeFocus);
}
